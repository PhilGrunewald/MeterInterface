...
def _connectDatabase(_dbHost):
    ...
    try:
        ...
        dbHost='localhost'
        dbConnection = MySQLdb.connect(host=dbHost, user=db.User, passwd= db.Pass, db=db.Name, cursorclass = MySQLdb.cursors.DictCursor)
        cursor = dbConnection.cursor()
    return cursor
...
def interact(screenKey, actKey, old_screenKey):
    ...
    i = 0
    title = screens['screens'][screenKey]['title']
    actDict["{}".format(title)] = {}
    for a in act:
        tucNext = acts['activities'][a]['ID']
        caption = acts['activities'][a]['caption']
        val = 'None'
        if ('value' in acts['activities'][a]):
            val = acts['activities'][a]['value']
            actDict["{}".format(title)]["{}".format(val)] = caption
        hits = ''
        if opt_hits:
...
def main(argv):
    ...
    nextScreen  = ['survey root', None, None]
    global opt_hits
    ...
    for opt, arg in opts:
       if opt in ("-h", "--help"):
          ...
          opt_hits = True
    while (nextScreen[0] != 'survey complete'):
        nextScreen = interact(nextScreen[0],nextScreen[1],nextScreen[2])
    print "Entry complete\n\n"
    with open('surveyKey.json', 'w') as outfile:
        json.dump(actDict, outfile)
